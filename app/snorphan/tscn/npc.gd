...
func _process(delta: float) -> void:
	...
	for i in range(15):
		...
		var horizontal_query = PhysicsRayQueryParameters3D.create(origin, origin + horizontal_target_direction *10)
		var vertical_query = PhysicsRayQueryParameters3D.create(origin, origin + vertical_target_direction *10)
